...
import sqlite3, pandas as pd, io, os
...
def add_produto():
    sku=request.form['sku']; titulo=request.form['titulo']; est=int(request.form['estoque'])
    con=db(); c=con.cursor()
...
def estoque():
    sku=request.form['sku']; qtd=int(request.form['quantidade']); tipo=request.form['tipo']
    con=db(); c=con.cursor()
    if tipo=='entrada':
    ...
    if request.method=='POST':
        ...
        for _,r in df.iterrows():
            c.execute("INSERT INTO vendas(sku,titulo,quantidade,receita,comissao,preco_medio) VALUES (?,?,?,?,?,?)",
                      (r['SKU'],r['Titulo'],int(r['Quantidade']),float(r['Receita']),float(r['Comissao']),float(r['PrecoMedio'])))
        con.commit(); con.close()
        flash("Importado!")
...
def exportar_template():
    ...
    return send_file(buf, as_attachment=True, download_name="template_consol.xlsx")
